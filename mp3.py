...
st.title("YouTube Batch Audio Downloader (Resume Support) 🎵")
...
if uploaded_file is not None:
    ...
    if urls:
        ...
        async def download_convert(url, idx):
            """Download YouTube audio and convert to MP3 asynchronously, skipping existing files."""
            try:
                ydl_opts = {
                ...
                # Convert to MP3
                await loop.run_in_executor(executor, lambda: AudioSegment.from_file(downloaded_file).export(mp3_filename, format="mp3"))
